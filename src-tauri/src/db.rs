...
impl Database {
    ...
    pub fn insert(&self, e: &HistoryEntry) -> Result<()> {
        self.conn.lock().unwrap().execute(
            ...
            params![
                e.id, e.url, e.title, e.thumbnail, e.duration,
                e.uploader, e.format_type, e.quality, e.actual_quality,
                e.size, e.output_path, e.downloaded_at
            ],
        )?;
        Ok(())
    }
